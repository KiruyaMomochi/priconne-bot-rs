...
use crate::{insight::AnnouncementPage, Error, Page};
...
impl AnnouncementPage for InformationPage {
    ...
    fn extra(&self) -> Self::ExtraData {
        Self::ExtraData { icon: self.icon }
    }

    fn tags(
        &self,
        tagger: &crate::insight::tagging::RegexTagger,
    ) -> linked_hash_set::LinkedHashSet<String> {
...
fn get_title(title_node: &NodeRef) -> Result<String, Error> {
    let title_node = title_node.first_child().ok_or(Error::KuchikiError)?;
