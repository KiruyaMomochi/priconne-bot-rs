    ...
    let recv = async move {
        while let Some(received) = rx.recv().await {
            let result = match &received {
                Command::Cartoon { id, chat_id } => match client.cartoon(*id).await {
                    Ok(cartoon) => telegram
                        .build()
                        .await?
                        .send_photo(chat_id.clone(), InputFile::url(url::Url::parse(&cartoon.image_src)?))
                        .send()
                        .await
                ...
                Command::News { id, chat_id } => todo!(),
                // bot
                //     .send_news_by_id(*id, chat_id.clone())
                //     .await
                //     .map_or_else(|error| Err(Error::from(error)), |_| Ok(())),
