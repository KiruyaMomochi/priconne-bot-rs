...
use cron::Schedule;
use priconne_core::{Client, Error, RegexTagger};
use reqwest::Url;
use scheduler::Schedules;
use serde::{Deserialize, Serialize};
use teloxide::{prelude::{Request, Requester, DependencyMap}, dptree::di::Injectable, utils::command::BotCommands};
...
impl TelegramConfig {
    ...
    //     telegramlisten_webhook(&listen_addr).await
    // }

    pub async fn repl<Cmd, H, Fut, E, Args>(&self, handler: H) -> Result<(), Error>
    where
...
impl BotConfig {
    pub async fn build(&self) -> Result<Bot<Client>, Error> {
        ...
        Ok(Bot::<Client> {
            ...
            database: db::Db::new(self.mongo.database().await?),
